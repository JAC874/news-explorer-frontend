...
function SavedNewsHeader({ articleCount, keywords }) {
  const { currentUser } = useContext(CurrentUserContext);
  const { userArticles } = useContext(UserArticleContext);

  // Filter articles saved by the logged-in user - use the logic below when not including stubbed data
  ...
  // Use the l ogic below to account for stubbed articles plus user articles
  // Combine stubbed and user-specific articles
  ...
  return (
    <section className="saved-news-header">
      ...
      <p className="saved-news-header__subtitle">
        ...
        <span className="saved-news-header__article-count">{articleCount}</span>{" "}
        saved article{articleCount !== 1 && "s"}.
      </p>
